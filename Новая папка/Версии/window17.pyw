...
def group_batches(signal):
        ...
        ##df15=df8.groupby(['Варка','Код'])['Квазипартия','Количество'].agg(lambda x: list(x)).reset_index()
        ##df15=df8.groupby(['Варка','Код'])['Квазипартия','Количество'].agg(lambda x: values.tolist(x)).reset_index()
        ##df15['Список']=df15.values.tolist() ##apply(lambda row: row['Квазипартия'].append(row['Количество']), axis=1)## Тут остановились
        ##df15['Список']=df15.values.tolist()
        df15=df15.apply(lambda row: [row['Варка'],row['Код'],list(zip(row['Квазипартия'],row['Количество']))], axis=1)
        ##apply(lambda row: row['Квазипартия'].append(row['Количество']), axis=1)## Тут остановились
        signal.emit(["Пишу",False,0,0,"", True,True,True]) 
        df15.to_excel('All7.xlsx',index=False,header=False)
        df8.to_excel('All8.xlsx',index=False,header=False)
