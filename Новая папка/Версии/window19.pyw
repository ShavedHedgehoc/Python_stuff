...
def group_batches(signal):
        ...
        df16 = pd.DataFrame((df15.apply(lambda row: [row['Варка'],row['Код']]+list(np.ravel(list(zip(row['Квазипартия'],row['Количество'])))), axis=1)).values.tolist())
        df16.columns = ['Варка', 'Код'] + df16.columns.tolist()[2:]
        common_cols= ['Варка', 'Код']
        df_res = pd.merge(df_varki,df16, on=common_cols, how='left') ## Выполняем левое соединение всего архива взвешиваний и уникальных комбинаций


        signal.emit(["Записываю файл результата",False,0,0,"", True,True,True]) 
        
        df_res.to_excel('All7.xlsx',index=False,header=True)
